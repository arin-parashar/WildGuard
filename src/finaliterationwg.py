...
class WildGuardPro:
    def __init__(self, root):
        ...
        try:
            self.esp = serial.Serial("COM4", 115200, timeout=1)
    ...
    def _animate_conf_to(self, target):
        self._conf_target = max(0.0, min(100.0, float(target)))
        if self._conf_anim_job is None:
            self._conf_tick()

    def _conf_tick(self):
        current = float(self.conf_bar["value"])
        target = self._conf_target
        ...
        self._conf_anim_job = self.root.after(24, self._conf_tick)

    # ================= LOGIC ENGINE =================
    def start_engine(self, mode):
        path = 0
        if mode == "file":
            path = filedialog.askopenfilename(filetypes=[("Media", "*.mp4 *.jpg *.png *.avi *.mov *.avif")])
            if not path: return
    ...
    def main_loop(self, path):
        if isinstance(path, str) and not path.lower().endswith(('.mp4', '.avi', '.mov')):
            ...
            while not self.stop_event.is_set():
                ...
                if not ret:
    # Force system reset when stream ends
                    try:
                        self.update_ui_stats(False, False)
                    except Exception as e:
                        print("UI error:", e)
                    ...
                    if self.esp:
                        try:
                            self.esp.write(b"idle\n")
                            print("Sent: idle (stream ended)")
                        except:
                            pass
                    self.animal_tracks.clear()
                    self.species_buffer.clear()
                    break
    ...
    def update_ui_stats(self, person_det, animal_det):
       ...
       # ================= UI UPDATE =================
        ...
        if not person_det and not animal_det:
            stage = "idle"
            status = "SYSTEM IDLE"
            color = ACCENT

        else:
            ui_threat = co_occ and category == "danger"
            stage = "threat" if ui_threat else "monitoring"
            status = "THREAT DETECTED" if ui_threat else "MONITORING ACTIVE"
            color = DANGER if ui_threat else ACCENT
